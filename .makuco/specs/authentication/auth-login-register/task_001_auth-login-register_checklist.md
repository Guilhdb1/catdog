# Makuco Codegen Checklist: TASK-AUTH-001 — Authentication core

**Purpose**: Validate the authentication core implementation for CatDog, including backend auth APIs, refresh rotation, email confirmation, password reset, and frontend auth pages.
**Created**: 2026-06-16
**Feature**: [spec_context.md](../spec_context.md)
**Prompt Plan**: task_001_auth-login-register.md
...
- SonarQube analysis could not run due to missing scanner runtime in the environment and Docker not available for the MCP SonarQube tool.
- `appsettings.json` contains a placeholder JWT secret that must be replaced for production use.
- Email sending is implemented as a no-op placeholder; a real transactional provider integration is needed to send confirmation and reset links.
